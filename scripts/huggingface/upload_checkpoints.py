"""Upload all DanceGRPO LoRA checkpoints from a folder to Hugging Face Hub.

Finds LoRA checkpoint directories (lora-checkpoint-*) inside the given folder
and uploads each one to a HF repo.

Usage:
    # Upload all LoRA checkpoints as subdirectories in one repo (default)
    python scripts/huggingface/upload_checkpoints.py \
        --checkpoints_dir /path/to/checkpoints \
        --repo_id username/my-dancegrpo-lora
    ...
    # Skip optimizer states to save space
    python scripts/huggingface/upload_checkpoints.py \
        --checkpoints_dir /path/to/checkpoints \
        --repo_id username/my-dancegrpo-lora \
        --skip_optimizer
...
CHECKPOINT_PATTERN = "lora-checkpoint-*"
...
def upload_checkpoints(
    ...
    ignore_patterns = []
    if skip_optimizer:
        ignore_patterns = list(OPTIMIZER_FILES)
        print(f"Skipping optimizer files: {ignore_patterns}")
        print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload DanceGRPO LoRA checkpoints to Hugging Face Hub"
    )
    parser.add_argument(
        "--checkpoints_dir",
        type=str,
        required=True,
        help="Path to the folder containing lora-checkpoint-* directories",
    )
    parser.add_argument(
        "--repo_id",
        type=str,
        required=True,
        help="HuggingFace repo ID (e.g. username/my-dancegrpo-lora)",
    )
    parser.add_argument(
        "--pattern",
        type=str,
    ...
    parser.add_argument(
        "--skip_optimizer",
        action="store_true",
        help="Skip lora_optimizer.pt files to save space",
    )
    parser.add_argument(
        "--private",
        action="store_true",
