...
step2_checkpoints() {
    ...
    echo "--- Download FLUX.1-dev to data/flux/ ---"
    echo "Run: huggingface-cli download black-forest-labs/FLUX.1-dev --local-dir data/flux"
    echo ""
    echo "--- Download HPS-v2.1 checkpoint ---"
    echo "Run: huggingface-cli download xswu/HPSv2 HPS_v2.1_compressed.pt --local-dir hps_ckpt"
    echo ""
    echo "--- Download CLIP ViT-H-14 checkpoint ---"
    echo "Run: huggingface-cli download laion/CLIP-ViT-H-14-laion2B-s32B-b79K open_clip_pytorch_model.bin --local-dir hps_ckpt"
    echo ""
    ...
    echo "    data/flux/transformer/              (FLUX transformer weights)"
    echo "    data/flux/vae/                       (FLUX VAE)"
    echo "    data/flux/text_encoder/              (CLIP-L)"
    echo "    data/flux/text_encoder_2/            (T5-XXL)"
    echo "    hps_ckpt/HPS_v2.1_compressed.pt"
    echo "    hps_ckpt/open_clip_pytorch_model.bin"
}
...
step3_preprocess() {
    ...
    echo "    Output: data/rl_embeddings/"
    echo ""

    bash scripts/preprocess/preprocess_flux_rl_embeddings_4gpus.sh

    echo ">>> STEP 3 complete. Verify data/rl_embeddings/videos2caption.json exists."
}
...
step4_train() {
    ...
    echo "    Output: data/outputs/grpo_baseline_4gpu_a6000/"
    echo ""
...
step5_evaluate() {
    ...
    echo ""
    echo "--- Reward curve ---"
    echo "The moving average reward is logged to ./reward.txt"
    ...
    echo ""
    echo "--- Visualization ---"
    echo "Generated images are saved to ./images/ during training."
    echo "For checkpoint visualization:"
    echo "  1. rm -rf data/flux/transformer/*"
    echo "  2. Copy trained LoRA checkpoint into data/flux/transformer/"
