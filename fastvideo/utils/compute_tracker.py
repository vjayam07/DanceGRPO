"""Training compute (FLOP) tracker for DanceGRPO experiments.

Uses PyTorch's built-in FlopCounterMode to measure actual FLOPs for each
phase of the GRPO training loop, then accumulates totals over the run.

On the first training step, each phase (sampling, VAE decode, reward,
GRPO gradient update) is profiled with FlopCounterMode to obtain per-
operation FLOP counts.  Subsequent steps multiply by operation counts
to avoid profiling overhead.

Metric names align with cfgrl-expo's ComputeTracker for comparison.

Usage:
    tracker = GRPOComputeTracker(num_gpus=4)

    # First step: calibrate by wrapping each phase
    with tracker.calibrate_sampling():
        run_sample_step(...)  # single forward pass
    with tracker.calibrate_vae_decode():
        vae.decode(...)
    with tracker.calibrate_reward():
        reward_model(...)
    with tracker.calibrate_training():
        grpo_one_step(...)  # single fwd+bwd
    ...
    tracker.record_grpo_training(num_samples, num_timesteps)
...
class GRPOComputeTracker:
    ...
    training_step_flops: int = 0       # One GRPO fwd+bwd (one timestep)
    ...
    # Wrap ONE invocation of each phase on the first step to measure FLOPs.
    ...
    def calibrate_training(self):
        """Wrap a single GRPO fwd+bwd pass (one timestep) to measure FLOPs."""
        counter = FlopCounterMode(display=False)
        with counter:
            yield
    ...
    def record_grpo_training(self, num_samples: int, num_timesteps: int):
        """Record the GRPO gradient update phase.

        Each sample trains on num_timesteps, each requiring a fwd+bwd pass.
        """
        self.total_training_flops += num_samples * num_timesteps * self.training_step_flops
    ...
    def summary(self) -> str:
        ...
        if self._calibrated:
            for name, was_cal in self._calibrated.items():
                lines.append(f"  {name}: measured" if was_cal else f"  {name}: NOT calibrated")
        return "\n".join(lines)
